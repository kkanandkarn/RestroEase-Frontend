...
const AddUserModal = ({ roles, onClose }) => {
  ...
  const addUser = async (imageUrl) => {
    try {
      ...
      const response = await CRUDAPI(ADD_USER, ADD_USER_METHOD, data, navigate);
      ...
      if (filePreview) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            const addUserResponse = await addUser(data.secure_url);
            setLoading(false);
            if (addUserResponse.status === "SUCCESS") {
              toast.success("User added successfully.");
              onClose(true, addUserResponse.data.users);
            }
          } else {
    ...
    >
      ...
      <div className="w-[70%] max-h-[60vh] bg-white border-2 border-zinc-400 rounded-lg shadow-lg m-auto flex flex-col">
        ...
        <div className="w-full h-14 flex gap-4 justify-end items-center border-t-2 border-t-zinc-400 p-4">
          <button
            className={`${
              loading ? "bg-zinc-400" : "bg-primaryColor"
